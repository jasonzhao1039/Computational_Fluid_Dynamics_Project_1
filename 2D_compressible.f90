...
!
  ...
  ! real(8)代表双精度；dimension(nx,ny)代表有nx行、ny列
   ! ro是ρ
...
990 format('vort',I4.4)
  ...
  itemp=1 !选择Adams-Bashforth scheme 1，还是 Runge-kutta scheme 2
...
!BEGINNING OF TIME LOOP
  do n=1,nt    !由n=1一直循环到n=nt（含 n=nt）
...
!######################################################################
  do j=1,ny
     do i=1,nx
        uuu(i,j)=uu0
        vvv(i,j)=0.01*(sin(4.*pi*(i)*dlx/xlx)& !扰动
             +sin(7.*pi*(i)*dlx/xlx))*&
             exp(-(j*dly-yly/2.)**2)
        tmp(i,j)=tpi
