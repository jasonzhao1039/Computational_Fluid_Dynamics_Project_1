...
!############################################
  ...
  dlx=xlx/nx
  udx=1./(12.*dlx)   !updated multiplier
  do j=1,ny
      dfi(1,j)=udx*(-phi(3,j)+8.*phi(2,j)-8.*phi(nx,j)+phi(nx-1,j))  !Boundary Condition: i=1
      dfi(2,j)=udx*(-phi(4,j)+8.*phi(3,j)-8.*phi(1,j)+phi(nx,j))  !Boundary Condition: i=2
     do i=3,nx-2
        dfi(i,j)=udx*(-phi(i+2,j)+8.*phi(i+1,j)-8.*phi(i-1,j)+phi(i-2,j))  !i=3 to nx-2
     enddo
     dfi(nx-1,j)=udx*(-phi(1,j)+8.*phi(nx,j)-8.*phi(nx-2,j)+phi(nx-3,j))  !Boundary Condition: i=nx-1
     dfi(nx,j)=udx*(-phi(2,j)+8.*phi(1,j)-8.*phi(nx-1,j)+phi(nx-2,j))  !Boundary Condition: i=nx
  enddo
...
!############################################
  ...
  dly=yly/ny !dly:grid size; yly: total length
  udy=1./(12.*dly)   !updated multiplier
  do j=3,ny-2
     do i=1,nx
        dfi(i,j)=udy*(-phi(i,j+2)+8.*phi(i,j+1)-8.*phi(i,j-1)+phi(i,j-2))  !j=3 to ny-2
     enddo
  enddo
  do i=1,nx !Boundary Condition
     dfi(i,1)=udy*(-phi(i,3)+8.*phi(i,2)-8.*phi(i,ny)+phi(i,ny-1))   !Boundary Condition: j=1
     dfi(i,2)=udy*(-phi(i,4)+8.*phi(i,3)-8.*phi(i,1)+phi(i,ny))   !Boundary Condition: j=2
     dfi(i,ny-1)=udy*(-phi(i,1)+8.*phi(i,ny)-8.*phi(i,ny-2)+phi(i,ny-3))  !Boundary Condition: j=ny-1
     dfi(i,ny)=udy*(-phi(i,2)+8.*phi(i,1)-8.*phi(i,ny-1)+phi(i,ny-2))  !Boundary Condition: j=ny
  enddo
...
!############################################
  ...
  dlx=xlx/nx
  udx=1./(12.*dlx*dlx)   !updated multiplier
  do j=1,ny
     dfi(1,j)=udx*(-phi(3,j)+16.*phi(2,j)-30.*phi(1,j)+16.*phi(nx,j)-phi(nx-1,j))   !Boundary Condition: i=1
     dfi(2,j)=udx*(-phi(4,j)+16.*phi(2,j)-30.*phi(2,j)+16.*phi(1,j)-phi(nx,j))   !Boundary Condition: i=2
     do i=3,nx-2
        dfi(i,j)=udx*(-phi(i+2,j)+16.*phi(i+1,j)-30.*phi(i,j)+16.*phi(i-1,j)-phi(i-2,j))  !i=3 to nx-2
     enddo
     dfi(nx-1,j)=udx*(-phi(1,j)+16.*phi(nx,j)-30.*phi(nx-1,j)+16.*phi(nx-2,j)-phi(nx-3,j))   !Boundary Condition: i=nx-1
     dfi(nx,j)=udx*(-phi(2,j)+16.*phi(1,j)-30.*phi(nx,j)+16.*phi(nx-1,j)-phi(nx-2,j))  !Boundary Condition: i=nx
   enddo
...
!############################################
  ...
  dly=yly/ny
  udy=1./(12.*dly*dly)   !updated multiplier
  do j=3,ny-2
     do i=1,nx
        dfi(i,j)=udy*(-phi(i,j+2)+16.*phi(i,j+1)-30.*phi(i,j)+16.*phi(i,j-1)-phi(i,j-2))  !j=3 to ny-2
     enddo
  enddo
  do i=1,nx
   dfi(i,1)=udy*(-phi(i,3)+16.*phi(i,2)-30.*phi(i,1)+16.*phi(i,ny)-phi(i,ny-1))  !Boundary Condition: j=1
   dfi(i,2)=udy*(-phi(i,4)+16.*phi(i,3)-30.*phi(i,2)+16.*phi(i,1)-phi(i,ny))  !Boundary Condition: j=2
   dfi(i,ny-1)=udy*(-phi(i,1)+16.*phi(i,ny)-30.*phi(i,ny-1)+16.*phi(i,ny-2)-phi(i,ny-3))  !Boundary Condition: j=ny-1
   dfi(i,ny)=udy*(-phi(i,2)+16.*phi(i,1)-30.*phi(i,ny)+16.*phi(i,ny-1)-phi(i,ny-2))  !Boundary Condition: j=ny
  enddo
